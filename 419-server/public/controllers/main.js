...
cscApp.controller('businessController', function($scope, $http) {
  function init () {
    $http.get('/api/reuseCategories').success(response => {
      $scope.categories = response;
      console.log(response);
    });
    $http.get('/api/repairCategories').success(response => {
  ...
  let refresh = () => {
    $http.get('/api/businesses').success((response)=>{
  ...
  $scope.addBusiness = () => {
    ...
    //if (validate ($scope.newBusiness,$scope.invalidAdd) == true)
      $http.post('/api/businesses', $scope.newBusiness).then((success) => refresh());
  }
  $scope.updateBusiness = () => {
    ...
    console.log($scope.editReuseCategories.slice(0));
        //if (validate($scope.selectedBusiness, $scope.invalid) == true)
          $http.put('/api/businesses/'+$scope.selectedBusiness._id, $scope.selectedBusiness).then((success) => refresh());
  }
  $scope.removeBusiness = id => {
    console.log(id);
    $http.delete('/api/businesses/'+id).then((success) => refresh());
    refresh();
  }
  $scope.clearNewBusiness = () => {
    $scope.newBusiness = {
